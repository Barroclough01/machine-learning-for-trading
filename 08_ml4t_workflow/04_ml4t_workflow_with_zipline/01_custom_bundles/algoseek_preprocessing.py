...
def get_equities():
    ...
    nasdaq_tickers = pd.DataFrame({"symbol": tickers_with_meta}).merge(
        nasdaq_info, how="left"
    )
    nasdaq_sids = equities.loc[
        equities.symbol.isin(nasdaq_tickers.symbol), ["symbol", "sid"]
    ]
    nasdaq_tickers = (
...
def get_dividends():
    ...
    dividends = read_sqlite("dividend_payouts", adjustments_con)[
        ["sid", "ex_date", "amount"]
    ]
    dividends = (
