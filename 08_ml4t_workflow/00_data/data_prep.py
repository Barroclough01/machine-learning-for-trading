...
PROJECT_DIR = Path("C:/Users/paxto/machine-learning-for-trading")
...
def get_backtest_data(predictions="lasso/predictions"):
    ...
    best_alpha = (
        predictions.groupby("alpha")
        .apply(lambda x: spearmanr(x.actuals, x.predicted)[0])
        .idxmax()
    )
    ...
    stop = (
        predictions.index.get_level_values("date").max() + pd.DateOffset(1)
    ).strftime("%Y-%m-%d")
    idx = pd.IndexSlice
